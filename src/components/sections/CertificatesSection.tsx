...
const individualCertificates = [
  ...
  { name: "Full Stack Java Development", image: certFullstackJava },
  { name: "Cybersecurity Fundamentals", image: certCybersecurity },
  { name: "Critical Infrastructure Protection", image: certCip },
  { name: "Saviynt Identity Security for AI Age", image: certSaviynt },
  { name: "Elite Tech Java Development", image: certElite1 },
];
...
const groupedCertificates = [
  {
    ...
    certificates: [
      { name: "Google Cloud Computing Foundations", image: certCloud1 },
  ...
  },
  {
    ...
    certificates: [
      { name: "Generative AI Fundamentals", image: certGenz1 },
      { name: "Advanced GenAI Applications", image: certGenz2 },
      { name: "GenAI for Business", image: certGenz3 },
      { name: "GenAI Ethics & Best Practices", image: certGenz4 }
    ]
  }
];

const IndividualCertificate = ({ cert }: { cert: typeof individualCertificates[0] }) => {
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden card-3d neon-glow group">
          ...
          <div className="p-4 bg-gradient-to-br from-primary/10 to-primary/5">
            ...
            <Button
              ...
              className="w-full bg-primary hover:bg-primary/90 text-primary-foreground hover:shadow-[0_0_20px_rgba(239,68,68,0.6)] transition-all duration-300"
            >
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-4xl w-[95vw] bg-background/95 backdrop-blur-lg border-2 border-primary shadow-[0_0_30px_rgba(239,68,68,0.5)] p-0">
          <button
            ...
            className="absolute top-4 right-4 z-50 text-primary hover:text-primary/80 transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
          <div className="p-4">
...
const GroupedCertificate = ({ group }: { group: typeof groupedCertificates[0] }) => {
  const [isOpen, setIsOpen] = useState(false);
  ...
  return (
    <section id="certificates" className="relative py-12 sm:py-16 md:py-20 px-4 sm:px-6 bg-gradient-to-b from-background to-primary/5">
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="mb-12">
          <ScrollReveal>
            <h3 className="text-2xl sm:text-3xl font-bold text-center mb-8 text-foreground glow-text">
              Individual Certifications
            </h3>
          </ScrollReveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {individualCertificates.map((cert, index) => (
              <IndividualCertificate key={index} cert={cert} />
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {groupedCertificates.map((group, index) => (
              <GroupedCertificate key={index} group={group} />
            ))}
          </div>
        </div>
      </div>
